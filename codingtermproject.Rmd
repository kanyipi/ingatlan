...
Here is the structure, but only the first twenty characters of the description as they get very long and it causes issues with the html.
```{r}
df %>% mutate(description=substr(description,1,20)) %>% head() %>% kable()
```

```{r}
df %>% mutate(area = as.factor(round_any(area,30))) %>% 
ggplot( aes(x = price, fill=area)) +
  geom_histogram(binwidth=10) + 
  xlim(0,300) +
    labs(x = "Price (in Millions HUF)", 
       y = "Count",
       title = "Price Histogram of Hungarian Properties on Sale") +
  theme_bw()
```

On this histogram we can see that the price distribution has it's majority between 0 and 100, and most of the properties there are rounded to 30 m2, 60 m2, and 90 m2.

```{r}
df %>% mutate(price = as.factor(round_any(price,30))) %>% 
...
df %>% 
  ...
  guides(colour = 'none') + 
  labs(x = "Property Condition", 
       y = "Price (in Millions HUF)",
       title = "Violin Plot of Prices in Different Property Conditions")
```

On this violin plot we can see a that the least properties are in befejezetlen which means not finished and that this category has the widest CI95. The average price for not finished (befejezetlen), newly built(új építésu), and like new (újszeru) are pretty same, with newly built being the highest.
...
```
On this violin plot we can see a interesting trend namely that the mean price of the properties built before 1950 is way higher than those of that were built between 1950 and 1980, and 1981 and 2000, then between 2001 and 2010 is a higher. We can also see that these categories are denser as these are intervals instead of years. Then from 2011 till 2022 the mean is not changing too much. Then it goes up for 2023, 2024, and 2025. My theory is that only expensive properties are planned in advance with multiple years are advertised, and cheaper properties are only advertised once they are built. NA (nincs megadva) has the lowest mean, this may mean that they do not know when it was built, or they just do not want to tell.

## Conclusion

In conclusion this is a very nice tool to scrape ingatlan.com as you wish.
