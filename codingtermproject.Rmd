...
This began as a full Hungary scrape but it turned into a functional ingatlan.com scrapper. One can give the link and the number of pages you want to scrape and it will scrape every property on the search link.
...
get_links_on_page which returns the absolute links from a ingatlan.com search page.

get_all_property which does the scraping of the given link and saves down the properties in batches

get_read_property which reads back the csvs to one data frame

get_clean which cleans the data

## Analysis

I wanted to show the structure, but it kills the html.
```{r}
head(df) %>% kable()
```

```{r}
df %>% mutate(area = as.factor(round_any(area,50))) %>% 
