...
This began as a full Hungary scraped but it turned into a functional ingatlan.com scrapper. One can give the link and the number of pages you want to scrape.
...
```{r}
df %>% 
  filter(!is.na(condition) & !is.na(price)) %>%
  mutate(condition=as.factor(condition)) %>% 
  ggplot(aes(x = condition, y = price, colour = condition)) + 
  ylim(0,300) +
  geom_violin() +
